...
test_batchsize=0.0003,60,5e-04,20,0.1,32,32），总需1小时26分，本
...
if __name__=='__main__': #为什么这部分代码一定要放在__main__块中？
    setup_seed(0) #尽管设置了种子，但是每次结果可能仍有稍许不同，如果去掉此行，每次结果则会有很大不同
    dataset_dir=os.path.join(os.path.dirname(__file__),'../images/Market-1501-v15.09.15/')
    checkpoint=CheckPoint()
    checkpoint.load('ResNet50_Classify.pth.tar') #允许随时中断训练进程
